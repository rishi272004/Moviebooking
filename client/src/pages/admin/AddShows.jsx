...
import { CheckIcon, DeleteIcon, Hand } from 'lucide-react'
...
const AddShows = () => {
    ...
    const [dateTimesSelection, setDateTimesSelection] = useState({})
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (!selectedMovie || Object.keys(dateTimesSelection).length === 0 || !showPrice) {
                return toast('Missing required fields')
            }

            const showsInput = Object.entries(dateTimesSelection).map(([date, time]) => ({ date, time }))

            const payload = {
                movieId: selectedMovie,
                showsInput, showPrice: Number(showPrice)
            }
        ...
        <>
            ...
            <div className='mt-6'>
                ...
                <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
                    ...
                    <button onClick={handleDateTimeAdd} className='bg-primary text-white px-3 py-2 text-sm rounded-lg hover:bg-primary/90 cursor-pointer'>
                        Add Time
                    </button>
                </div>
            </div>
            ...
            {Object.keys(dateTimesSelection).length > 0 && (
                <div className='mt-6'>
                    <h2 className='mb-2'>Selected Date-Time</h2>
                    <ul className='space-y-3'>
                        {Object.entries(dateTimesSelection).map(([date, times]) => (
                            <li key={date}>
